...
public interface PromocaoRepository extends JpaRepository<Promocao, Long> {
    ...
    // porque Query é consulta, mas estamos usando update.
    // Precisa entao do @Transactional e @Modifying
    @Transactional(readOnly = true)
    @Modifying
    @Query("update Promocao p set p.likes = p.likes + 1 where p.id = :id" )
    void updateSumLikes(@Param("id") Long id);
    ...
    Page<LocalDateTime> findLastPromotionDate(Pageable pageable);

    @Query("select count(p.id) as count, max(p.dtCadastro) as lastDate " +
            " from Promocao p where p.dtCadastro > :data ")
    Map<String, Object> totalAndLastPromoByDataCadastro(@Param("data") LocalDateTime data);
}
